...
from timm.data import create_loader, resolve_data_config, Mixup, FastCollateMixup, \
    AugMixDataset
...
parser.add_argument('-vb', '--validation-batch-size', type=int, default=None,
                    metavar='N',
...
parser.add_argument('--head-dropout', type=float, default=0.0, metavar='PCT',
...
def main():
    ...
    if args.mpi:
        ...
        args.local_rank = world_rank % gpu
        args.device = 'cuda:%d' % args.local_rank
        torch.cuda.set_device(args.local_rank)
        torch.distributed.init_process_group(backend='nccl',
                                             init_method=f'tcp://{host[0]}:50000',
    ...
    if args.rank == 0:
        ...
        if args.log_clearml:
            if has_clearml:
                ...
                _logger.warning(
                    "You've requested to log metrics to clearml but package not found. "
    ...
    if 'convformer' in args.model or 'caformer' in args.model or 'gfformer' in args.model or 'dfformer' in args.model:
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                ...
                if args.log_clearml and args.rank == 0:
                    clearml.Logger.current_logger().report_scalar("train_iter",
                                                                  "avg_loss",
                                                                  iteration=batch_idx + len(
                                                                      loader) * epoch,
                                                                  value=losses_m.avg)
                ...
                if args.save_images and output_dir:
                    torchvision.utils.save_image(
                        ...
                        os.path.join(output_dir,
                                     'train-batch-%dPlease specify the project name..jpg' % batch_idx),
                        padding=0,
                        normalize=True)
